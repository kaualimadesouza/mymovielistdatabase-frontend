...
export function Movies() {
  ...
  function nextPage() {
    navigate(`/movies/page/${parseInt(realPage) + 1}`);
    location.reload();
  }
    ...
    <div className="h-[100vh]">
      ...
      <main className="bg-custom-gradient py-11">
        <div className="w-full space-y-3 max-w-[980px] m-auto h-[880px] relative">
          ...
          <div></div>
          <div className="grid grid-cols-main justify-between gap-2 border-b-[1px] pb-3 border-zinc-100 border-opacity-25">
            {movieData.map((movie) => (
              <div
                key={movie.id}
                className="relative
...
before:rounded-md before:hidden
...
hover:before:block hover:after:block
"
          ...
          {realPage === "1" && totalMovies.length < 72 ? (
            <div>
              <button
                onClick={nextPage}
